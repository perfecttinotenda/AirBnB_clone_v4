#!/usr/bin/python3
'''This will contain the places_amenities view for our API.'''
...
def handle_places_amenities(place_id=None, amenity_id=None):
    '''Our method handler for the places endpoint.'''
    handlers = {
...
def get_place_amenities(place_id=None, amenity_id=None):
    '''Will get amenity per given id or all of the amenities. of a place per given id.'''
    if place_id:
...
def remove_place_amenity(place_id=None, amenity_id=None):
    '''This removes a amenity per given id from a place per given id.'''
...
def add_place_amenity(place_id=None, amenity_id=None):
    '''This updates the amenity per given id to a place per given id.'''
    if place_id and amenity_id:
        place = storage.get(Place, place_id)
        if not place:
            raise NotFound()
