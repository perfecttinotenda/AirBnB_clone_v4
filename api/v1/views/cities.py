#!/usr/bin/python3
'''This will contain the cities view for our API.'''
...
def handle_cities(state_id=None, city_id=None):
    '''Our method handler of cities endpoint.'''
    handlers = {
        'GET': get_cities,
        'DELETE': remove_city,
        'POST': add_city,
...
def get_cities(state_id=None, city_id=None):
    '''Will get city per given id or all of the cities.'''
    if state_id:
...
def remove_city(state_id=None, city_id=None):
    '''This removes a city per given id.'''
    if city_id:
        city = storage.get(City, city_id)
        if city:
            storage.delete(city)
