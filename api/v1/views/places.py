#!/usr/bin/python3
'''This will contain the places view for our API.'''
...
def handle_places(city_id=None, place_id=None):
    '''Our method handler of places endpoint.'''
    handlers = {
        'GET': get_places,
        'DELETE': remove_place,
        'POST': add_place,
...
def get_places(city_id=None, place_id=None):
    '''Will get place per given id or all of the place.'''
    if city_id:
...
def remove_place(city_id=None, place_id=None):
    '''This will remove a place per given id.'''
    if place_id:
        place = storage.get(Place, place_id)
        if place:
...
def update_place(city_id=None, place_id=None):
    '''This will update the place per given id.'''
...
def find_places():
    '''This will find places based per list of State, City, or Amenity ids.'''
    data = request.get_json()
    if type(data) is not dict:
        raise BadRequest(description='Not a JSON')
