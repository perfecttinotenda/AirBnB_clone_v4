#!/usr/bin/python3
'''This will contain the states view for our API.'''
...
def handle_states(state_id=None):
    '''Our method handler of the states endpoint.'''
    handlers = {
        'GET': get_states,
        'DELETE': remove_state,
        'POST': add_state,
...
def get_states(state_id=None):
    '''This will get the state per given id or all of the states.'''
...
def remove_state(state_id=None):
    '''This will remove a state per given id.'''
    all_states = storage.all(State).values()
    res = list(filter(lambda x: x.id == state_id, all_states))
    if res:
...
def update_state(state_id=None):
    '''This will update the state per given id.'''
